...
void loop() {
  if (accel.available()) {      // Wait for new data from accelerometer
    ...
    float xAcc = accel.getCalculatedX();
    float yAcc = accel.getCalculatedY();
    float zAcc = accel.getCalculatedZ();
    
    if(xAcc > 1.5){Serial.print("big +X\n");}
    if(xAcc < -1.5){Serial.print("big -X\n");}
    
    Serial.print("\t");
    if(yAcc > 1.5){Serial.print("big +Y\n");}
    if(yAcc < -1.5){Serial.print("big -Y\n");}
    
    
    Serial.print("\t");
    if(zAcc > 1.5){Serial.print("big +Z\n");}
    if(zAcc < -1.5){Serial.print("big -Z\n");}
    //Serial.println();
  }
}
